...
# Load ResNet-101 and DenseNet models
RESNET_MODEL_PATH = "models/resnet101_optimized.h5"
DENSENET_MODEL_PATH = "models/densenet_optimized.h5"
...
# Function to classify an image using both models
def classify_image(image_path):
    resnet_prediction, resnet_confidence = "Error", 0
    densenet_prediction, densenet_confidence = "Error", 0
    ...
    # DenseNet Prediction (380x380)
    if densenet_model:
        try:
            densenet_image = preprocess_image(image_path, 380)
            densenet_predictions = densenet_model.predict(densenet_image)
            densenet_predicted_class = np.argmax(densenet_predictions, axis=1)[0]
            densenet_confidence = np.max(densenet_predictions)
            densenet_prediction = CLASS_LABELS[densenet_predicted_class]
        except Exception as e:
            print(f" DenseNet Prediction Error: {e}")
...
def predict():
    ...
    if file and allowed_file(file.filename):
        ...
        # Convert confidence values to float and round percentages
        return jsonify({
            "resnet_prediction": predictions["resnet_prediction"],
            "resnet_confidence": round(float(predictions["resnet_confidence"]) * 100, 2) if predictions["resnet_confidence"] else "Error",
            "densenet_prediction": predictions["densenet_prediction"],
            "densenet_confidence": round(float(predictions["densenet_confidence"]) * 100, 2) if predictions["densenet_confidence"] else "Error",
